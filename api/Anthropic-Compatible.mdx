...
| 参数            | 类型                                | 是否必需 | 说明                                         |
| ------------- | --------------------------------- | ---- | ------------------------------------------ |
| `model`       | string                            | 是    | 支持模型请查询[模型列表](https://camel.kr777.top/models) |
...
虽然您可以通过 CaMeL AI 使用任何模型，但为了获得最佳体验，我们建议您使用功能强大的模型，例如 gpt-5.2、glm-4.7  等。

#### 1. 安装配置 Claude Code

Claude Code 安装请参考：[/api/Claude-Code](/api/Claude-Code)

#### 2. 将 Claude Code 连接到CaMeL AI
...
{
  "env": {
    "ANTHROPIC_API_KEY": "CAMELAI_API_KEY",
    "ANTHROPIC_BASE_URL": "https://camel.kr777.top",
...
请替换 `CAMELAI_API_KEY` 为实际的 CaMeL AI API 密钥。
...
连接成功后，您发送的任何提示都将通过 CaMeL AI 路由。
...
> /status
 ...
 Anthropic base URL: https://camel.kr777.top
