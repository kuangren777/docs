...
1️⃣ 对于原生调用，我们的 Gemini 调用支持 AI Studio 和 VertexAI 自动路由。转发方法主要是在内部传入 CaMeL AI 密钥和请求链接。需要注意的是，这个链接和常规的 `base_url` 写法不同，请参考示例：
...
<CodeGroup>
  ...
  response = client.chat.completions.create(
      ...
      tool_choice="auto" ## 📍 此处追加了 CaMeL AI 兼容，更稳定的请求方式
  )
