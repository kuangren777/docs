---
title: "CaMeL AI 三方接入技术指南"
description: "基于 `@camelai/ai-sdk-provider`、`dify-plugin-camelai` 和 `cherry-studio` 三个项目的核心实现逻辑，本文档为开发者提供快速接入 CaMeL AI 统一网关的技术指南。"
---

>  基于 `@camelai/ai-sdk-provider`、`dify-plugin-camelai` 和 `cherry-studio` 三个项目的核心实现逻辑，本文档为开发者提供快速接入 CaMeL AI 统一网关的技术指南。 
...
<Tabs>
  ...
  <Tab title="Python">
    ...
    # 核心逻辑：替换 API Key、Base URL，添加 APP-Code
    credentials = {
        "api_key": "your-camelai-api-key",  # 替换为 CaMeL AI API Key
        "base_url": "https://camel.kr777.top",   # 替换为 CaMeL AI 网关
        "extra_headers": {
            "APP-Code": "APP Code"  # 应用Code可从https://camel.kr777.top/appstore获取
        }
    }
  ...
  <Tab title="Python">
    ...
    class CaMeL AIModelClient:
        def __init__(self, api_key: str):
            ...
            self.base_url = "https://camel.kr777.top"
            self.headers = {
    ...
    # 使用示例
    client = CaMeL AIModelClient("your-camelai-api-key")
...
<Tabs>
  ...
  <Tab title="Python">
    ...
    config = {
        "api_key": os.getenv("CAMELAI_API_KEY", ""),
        "base_url": os.getenv("CAMELAI_BASE_URL", "https://camel.kr777.top"),
        "app_code": os.getenv("CAMELAI_APP_CODE", "APP Code")
    }
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Python">
    ```python  theme={null}
    class CaMeL AIError(Exception):
        def __init__(self, message: str, code: str = None, status: int = None):
            super().__init__(message)
            self.code = code
            self.status = status

    def handle_camelai_errors(e: Exception) -> CaMeL AIError:
        message = str(e).lower()
        if "rate limit" in message:
            return CaMeL AIError("Rate limit exceeded", "RATE_LIMIT", 429)
        elif "unauthorized" in message:
            return CaMeL AIError("Authentication failed", "AUTH_ERROR", 401)
        else:
            return CaMeL AIError(f"Request failed: {e}")
    ```
  </Tab>
</Tabs>
