---
title: "视频生成接口"
description: "CaMeL AI 提供统一的视频生成 API，兼容 OpenAI Sora 接口格式，后端支持多家厂商模型"
---

>  CaMeL AI 提供统一的视频生成 API，兼容 OpenAI Sora 接口格式，后端支持多家厂商模型
...
# 第一步：提交视频生成任务
curl -X POST https://api.kr777.top/videos \
...
curl https://api.kr777.top/videos/{video_id} \
  -H "Authorization: Bearer $CAMELAI_API_KEY"

# 第三步：下载视频
curl https://api.kr777.top/videos/{video_id}/content \
  -H "Authorization: Bearer $CAMELAI_API_KEY" \
  --output video.mp4
```
...
Base URL：`https://camel.kr777.top`
...
```json  theme={null}
{
  ...
  "width": 1920,
  "height": 1080,
  "url": "https://api.kr777.top/videos/eyJtb2RlbCI6IndhbjI.../content",
  "error": null
}
```
...
<CodeGroup>
  ...
  const API_KEY = "your_camelai_api_key";
  const BASE_URL = "https://camel.kr777.top";
